...
let username = ''; // Store the user's email or name
...
const replyBox = document.getElementById('reply-box'); // New reply box
...
function sendMessage() {
  if (messageInput.value.trim() !== '' && username !== '') {
    ...
    replyBox.innerHTML = ''; // Clear the reply box
  }
}
...
function replyToMessage(messageText) {
  ...
  replyBox.innerHTML = `Replying to: "<i>${messageText}</i>" <button onclick="cancelReply()">Cancel</button>`;
}
...
socket.on('chat message', (msg) => {
  ...
  if (msg.replyTo) {
    ...
    quotedMessage.innerHTML = `<i>Replying to: "${msg.replyTo}"</i>`;
  ...
  if (msg.user === username) {
    newMessage.classList.add('sent-message'); // Style messages from the current user
  } else {
    newMessage.classList.add('received-message'); // Style messages from others
  }
